...
writecode(data, multiple, length, name, sleepdivider, sleepsuber)
{
	...
	;FileAppend , %text%`n, code.txt, UTF-8
	text := "			sleepdivider := "
	FileAppend , %text%%sleepdivider%`n, code.txt, UTF-8
	text := "			sleepsuber := "
	FileAppend , %text%%sleepsuber%`n, code.txt, UTF-8
	...
	while(i <= length*multiple)
	{
		...
		else
		{
			if(data[i,2] > 0)
				...
				flag := ""
			text := "				"
			FileAppend , %text%DllCall("mouse_event", code.txt, UTF-8
			text := ", "
			FileAppend , %text%"UInt", code.txt, UTF-8
			text := ", "
			FileAppend , %text%1, code.txt, UTF-8
			text := ", "
			FileAppend , %text%"UInt", code.txt, UTF-8
			text := ", "
			text2 := data[i,1]
			FileAppend , %text%%text2%, code.txt, UTF-8
			text := ", "
			FileAppend , %text%"UInt", code.txt, UTF-8
			text := ", "
			text2 := abs(data[i,2])
			FileAppend , %text%%flag%%text2%)`n, code.txt, UTF-8
			if(i <= multiple)
			...
			text := "				"
			FileAppend , %text%x := x %flag% %b1%`n, code.txt, UTF-8
			if(data[i,2]>0)
			{
				flag := "+"
				b2 := abs(data[i,2])
			...
			text := "				"
			FileAppend , %text%y := y %flag% %b2%`n, code.txt, UTF-8
		}
		if(i >= 2*multiple)
		{
			if(mod(i, multiple) == 0)
			{
				text := "				"
				FileAppend , %text%x1 := x / modifier2 + width / 2 - thickness / 2`n, code.txt, UTF-8
				FileAppend , %text%y1 := y / modifier2 + height / 2 - thickness / 2`n, code.txt, UTF-8
		...
		if(i < length * multiple)
		{
			...
			text := "				{"
			FileAppend , %text%`n, code.txt, UTF-8
			text := "					recover(x, y, x0, y0, x1, y1, x2, y2, tsleep, tsleep0, tsleep2, tsleep3, round, speed)"
			FileAppend , %text%`n, code.txt, UTF-8
			text := "					break"
			FileAppend , %text%`n, code.txt, UTF-8
			text := "				}"
